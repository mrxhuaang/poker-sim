...
| 2026-06-07 | **EPIC 3 — Layout dos columnas para desktop (lg:grid-cols-[1fr_320px]).** Felt (board + pot grande + ganadores) a la izquierda; seats + dock de apuestas a la derecha. `prefers-reduced-motion` respetado vía clases `motion-safe:`. | EPIC 0 garantiza que online/torneo solo corren en desktop (≥1024px), así que podemos asumir landscape y maximizar el espacio horizontal. |
| 2026-06-07 | **EPIC 3 — `OnlineBettingControls`: presets de pot (1/3, 1/2, 2/3, Pot, All-in) + slider.** Estado de raise con patrón `amountKey` para reset limpio en cada nueva ronda. Amount = raise-to total (chips comprometidas esta calle desde 0). | Mismo patrón que `BettingControls` legacy; evita `useEffect` para reset de estado. La validación real vive en el Go server. |
| 2026-06-07 | **EPIC 3 — `OnlineTurnTimer`: barra de progreso usando `useCountdown`.** 30s hardcoded (coincide con `turnTimeout` en `session/manager.go`). Colores: accent (>50%) → warn (25-50%) → rose (<25%). | Reusa el hook `useCountdown` con rAF throttling; sin polling extra. |
| 2026-06-07 | **EPIC 3 — `WaitingPlaceholder` en el felt.** Detecta: conectando/reconectando (spinner), < 2 jugadores activos, listo para repartir. | Estados de espera sin elementos vacíos — reduce confusión de usuario nuevo. |
| 2026-06-07 | **EPIC 4 — Supabase Postgres como store autoritativo de manos.** Tabla `online_hand_records`; RLS: SELECT público, INSERT solo service_role. | El servidor Go escribe; el cliente solo lee. Evita Firebase Admin SDK en Go. |
| 2026-06-07 | **EPIC 4 — `server/internal/store/supabaseStore`: HTTP client sin driver.** POST a `/rest/v1/online_hand_records` con service role key. nil-safe (no-op si env vars ausentes). | Sin dependencias Go extra; Supabase REST es una API HTTP estándar. El servidor ya hace HTTP (auth Firebase). |
| 2026-06-07 | **EPIC 4 — `HandCategories map[string]int` en `PublicState`.** Computado en `room.computeHandCategories()` después de `reveals` en ambos paths (showdown y run-it-N). Categoría = `score >> 20` (0-8). | El cliente ahora tiene categoría real en lugar del placeholder "High Card". |
| 2026-06-07 — **EPIC 4 — Persistencia post-showdown en `session.Manager`.** `buildRecord()` snapshottea el room (getters `Board()`, `Reveals()`, `Pot()`, `SeatNames()`); `persistHand()` llama al store en goroutine (fire-and-forget). | Async para no bloquear el broadcast; fallos se loggean pero no crashean el server. |
| 2026-06-07 | **EPIC 4 — `useOnlineHistory`: fetch desde Supabase, re-trigger por `handNum` en showdown.** Panel en `ServerTable` muestra 10 manos recientes con ganador, categoría, pot. | `triggerKey = handNum` cuando phase=showdown asegura re-fetch exactamente cuando hay datos nuevos, sin polling. |
